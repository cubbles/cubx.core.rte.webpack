...
import rootDependencies from './dependencyResolution/rootDependencies';
...
describe('DepMgrDependencyResolution', function () {
  ...
  describe('#_calculateResourceList()', function () {
    ...
    beforeEach(function () {
      ...
      item3.resources = ['test3-1.js'];
      internalDepList.push(item1);
      internalDepList.push(item2);
      internalDepList.push(item3);
    });
    it('should return a list of all resources in correct order for given list of DepReference items', function () {
      ...
      resourceList[0].should.have.property(
        'path',
        depMgr._baseUrl +
          item1.webpackageId +
          '/' +
      ...
      resourceList[1].should.have.property(
        'path',
        depMgr._baseUrl +
          item1.webpackageId +
          '/' +
      ...
      resourceList[2].should.have.property(
        'path',
        depMgr._baseUrl +
          item1.webpackageId +
          '/' +
      ...
      resourceList[3].should.have.property(
        'path',
        depMgr._baseUrl +
          item2.webpackageId +
          '/' +
      ...
      resourceList[4].should.have.property(
        'path',
        depMgr._baseUrl +
          item2.webpackageId +
          '/' +
      ...
      resourceList[5].should.have.property(
        'path',
        depMgr._baseUrl +
          item2.webpackageId +
          '/' +
  ...
  describe('#_createResourceFromItem()', function () {
    ...
    it('should add the baseUrl as prefix for all files if some url is given', function () {
      ...
      var resource = depMgr._createResourceFromItem(id, item, 'prod');
      resource.should.have.property(
        'path',
        depMgr._baseUrl + id + '/' + item.prod
      );
    ...
    describe('resources with absolute url', function () {
      describe('allowAbsoluteResourceUrls is false', function () {
        ...
        it('should not create a new resource for item given as string using an absolute url, since allowAbsoluteResourceUrls = false', function () {
          ...
          var resource = depMgr._createResourceFromItem(
            id,
            absoluteUrlString,
            'prod'
          );
          ...
          consoleWarnSpy.should.be.calledWith(
            'The following resource can not be loaded since the use of absolute urls is not allowed by default: blob:http://xxxxxx?type=js'
          );
        });
      });
        ...
        describe('not create resource', function () {
          ...
          it('should not create a new resource and return undefined, if the type valu is unkown', function () {
            ...
            spyConsoleWarn.calledWith(
              'The following resource will be ignored, because the type of the resource is unkown. (blob:http://xxxxxx?type=xxx)'
            );
          });
          it('should not create a new resource and return undefined, if the type parameter missed', function () {
            ...
            spyConsoleWarn.calledOnce; // eslint-disable-line chai-friendly/no-unused-expressions
            spyConsoleWarn.calledWith(
              'The following resource will be ignored, because the type of the resource is unkown. (blob:http://xxxxxx?yyy=xxx)'
            );
          });
          it('should not create a new resource and return undefined, if no paramter exists', function () {
            ...
            spyConsoleWarn.calledOnce; // eslint-disable-line chai-friendly/no-unused-expressions
            spyConsoleWarn.calledWith(
              'The following resource will be ignored, because the type of the resource is unkown. (blob:http://xxxxxx)'
            );
          });
        });
      });
  ...
  describe('#_fetchManifest()', function () {
    ...
    before(function () {
      depMgr = window.cubx.CRC.getDependencyMgr();
      axiosStub = sinon
        .stub(Object.getPrototypeOf(depMgr._axios), 'request')
    ...
    it('should call axios.request method with given url', function (done) {
      depMgr._fetchManifest('https://www.example.test').then(function () {
        expect(
          axiosStub.calledWith({ url: 'https://www.example.test' })
        ).to.be.true;
        done();
      });
    });
  ...
  describe('#_getDependencyListFromTree', function () {
    ...
    it('should return an array of DepReference items', function () {
      ...
      list.forEach(function (item) {
        item.should.be.an.instanceOf(DepMgr.DepReference);
      });
    });
  ...
  describe('#_getIndexOfDepReferenceItem()', function () {
    ...
    before(function () {
      ...
      artifactDependencies = rootDependencies;
      depMgr.init();
    });
    it('should return index of DepReference item in internal depList', function () {
      var depReferences = depMgr._createDepReferenceListFromArtifactDependencies(
        artifactDependencies,
        testReferrer
      );
      depMgr._depList = depReferences;
      expect(
        depMgr._getIndexOfDepReferenceItem(depMgr._depList, depReferences[0])
      ).to.equal(0);
    });
    it('should return -1, as given item is not in internal depList', function () {
      ...
      expect(
        depMgr._getIndexOfDepReferenceItem(depMgr._depList, items[0])
      ).to.equal(-1);
    });
    after(function () {
      depMgr._depList = null;
  ...
  describe('#_getManifestForDepReference()', function () {
    ...
    beforeEach(function () {
      ...
      fetchManifestStub = sinon
        .stub(Object.getPrototypeOf(depMgr), '_fetchManifest')
        .callsFake(function (url) {
          return new Promise(function (resolve, reject) {
    ...
    it('should return a promise', function () {
      expect(
        depMgr._getManifestForDepReference(depRefItem, baseUrl)
    ...
    it('should resolve returned promise with manifest from responseCache if there is one', function () {
      ...
      depMgr._responseCache.addItem('package1@1.0.0', pkg1);
      return depMgr
        ._getManifestForDepReference(depRefItem)
        .then(function (result) {
          result.should.be.eql(pkg1);
          expect(getItemFromCacheSpy.calledOnce);
          expect(fetchManifestStub.callCount).to.be.equal(0);
        });
    });
    it(
      'should resolve returned promise with inline manifest from depReference if there is one ' +
        'and no manifest was found in responseCache',
      function () {
    ...
    it(
      'should resolve returned promise with requested manifest from baseUrl if there is ' +
        'neither a manifest in responseCache nor in inline manifest',
      function () {
        return depMgr
          ._getManifestForDepReference(depRefItem, baseUrl)
          .then(function (result) {
            result.should.be.eql(pkg1);
            expect(getItemFromCacheSpy.calledOnce);
            expect(fetchManifestStub.calledOnce);
            expect(
              fetchManifestStub.calledWith(
                baseUrl + 'package1@1.0.0/webpackage.manifest'
              )
            );
          });
      }
    ...
    describe('Error handling', function () {
      it('should reject returned promise if there is an error while fetching manifest', function () {
        ...
        return depMgr._getManifestForDepReference(depRefItem, baseUrl).then(
          function (resolved) {
      ...
      it('should reject returned promise if baseUrl is not given when fetching manifest needs to be called', function () {
        return depMgr._getManifestForDepReference(depRefItem).then(
          function (resolved) {
            throw new Error('Promise was unexpectedly fulfilled: ', resolved);
          },
          function (rejected) {

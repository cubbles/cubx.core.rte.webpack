...
import DependencyTree from '../../modules/dependencyTree/dependencyTree';
import rootDeps from './dependencyResolution/rootDependencies';
import pkg1 from './dependencyResolution/dependencyPackage1';
import pkg2 from './dependencyResolution/dependencyPackage2';
import pkg3 from './dependencyResolution/dependencyPackage3';
import pkg4 from './dependencyResolution/dependencyPackage4';
import pkg5 from './dependencyResolution/dependencyPackage5';
import pkg6 from './dependencyResolution/dependencyPackage6';
...
describe('DependencyMgr DependencyTree creation', function () {
  describe('#_buildRawDependencyTree()', function () {
    ...
    before(function () {
      ...
      window.cubx.CRCInit.rootDependencies = rootDeps;
      depMgr = window.cubx.CRC.getDependencyMgr();
      depMgr.init();
      baseUrl = depMgr._baseUrl;

      stub = sinon
        .stub(depMgr, '_resolveDepReferenceDependencies')
        .callsFake(function (dep) {
            ...
            if (requestedPkg) {
              window.setTimeout(function () {
                resolve({
                  ...
                  resources: requestedPkg.artifacts.utilities[0].resources
                });
              }, 200);
    ...
    it('should return a promise', function () {
      expect(
        depMgr._buildRawDependencyTree(rootDepList, baseUrl)
      ).to.be.an.instanceOf(Promise);
    });
    it('should resolve the returned promise with an instance of DependencyTree', function () {
      return depMgr
        ._buildRawDependencyTree(rootDepList, baseUrl)
    ...
    it('should create a DependencyTree representing the raw dependency structure for given rootDependency list', function () {
      ...
      return depMgr
        ...
        .then(function (tree) {
          ...
          expect(tree._rootNodes[0].data.getId()).to.equal(
            'package1@1.0.0/util1'
          );
          expect(tree._rootNodes[1].data.getId()).to.equal(
            'package2@1.0.0/util2'
          );
          // check second level of tree
          expect(tree._rootNodes[0].children[0].data.getId()).to.equal(
            'package3@1.0.0/util3'
          );
          expect(tree._rootNodes[0].children[1].data.getId()).to.equal(
            'package4@1.0.0/util4'
          );
          expect(tree._rootNodes[1].children[0].data.getId()).to.equal(
            'package3@1.0.0/util3'
          );
          expect(tree._rootNodes[1].children[1].data.getId()).to.equal(
            'package5@1.0.0/util5'
          );
          // check third level of tree
          expect(
            tree._rootNodes[0].children[0].children[0].data.getId()
          ).to.equal('package5@1.0.0/util5');
          expect(
            tree._rootNodes[1].children[0].children[0].data.getId()
          ).to.equal('package5@1.0.0/util5');
          expect(
            tree._rootNodes[1].children[1].children[0].data.getId()
          ).to.equal('package6@1.0.0/util6');
          // check 4th level of tree
          expect(
            tree._rootNodes[0].children[0].children[0].children[0].data.getId()
          ).to.equal('package6@1.0.0/util6');
          expect(
            tree._rootNodes[1].children[0].children[0].children[0].data.getId()
          ).to.equal('package6@1.0.0/util6');
        });
    });
    describe('Error Handling', function () {
      ...
      it('should reject returned promise if there is an TypeError resolving single dependencies', function () {
        ...
        return depMgr._buildRawDependencyTree(rootDepList, baseUrl).then(
          function (result) {
            throw new Error(
  ...
  describe('#_checkAndAddExcludesForRootDependencies', function () {
    ...
    beforeEach(function () {
      ...
      node.data = new DepMgr.DepReference({
        artifactId: 'util1',
        webpackageId: 'package1@1.0.0',
        referrer: null
      });
  ...
  describe('#_checkDepTreeForExcludes()', function () {
    ...
    beforeEach(function () {
      ...
      spy = sinon.spy(
        Object.getPrototypeOf(depMgr),
        '_checkAndAddExcludesToDepReference'
      );

      // spy _checkAndAddExcludesForRootDependencies()
      spy2 = sinon.spy(
        Object.getPrototypeOf(depMgr),
        '_checkAndAddExcludesForRootDependencies'
      );

      // stub _getManifestForDepReference()
      stub = sinon
        .stub(Object.getPrototypeOf(depMgr), '_getManifestForDepReference')
        .callsFake(function (depRefItem, baseUrl) {
          var manifest;
          switch (depRefItem.webpackageId) {
            case 'package1@1.0.0':
              ...
              manifest.artifacts.utilities[0].dependencyExcludes = [
                {
                  webpackageId: 'anotherPackageExclude',
                  artifactId: 'anotherArtifactExclude'
                }
              ];
              break;
            case 'package3@1.0.0':
      ...
      root.data = new DepMgr.DepReference({
        webpackageId: 'package1@1.0.0',
        artifactId: 'util1',
        referrer: null
      });
    ...
    it('should call _checkAndAddExcludesToDepReference() for each Node in DependencyTree and assign dependencyExcludes if there are any', function () {
      return depMgr._checkDepTreeForExcludes(depTree, baseUrl).then(function () {
        ...
        depTree._rootNodes[0].data.should.have.ownProperty(
          'dependencyExcludes'
        );
        ...
        depTree._rootNodes[0].children[0].data.should.have.ownProperty(
          'dependencyExcludes'
        );
  ...
  describe('#_resolveDepReferenceDependencies()', function () {
    ...
    before(function () {
      ...
      _fetchManifestStub = sinon
        .stub(depMgr, '_fetchManifest')
    ...
    it('should return a promise', function () {
      expect(
        depMgr._resolveDepReferenceDependencies(depRefItem, baseUrl)
      ).to.be.an.instanceOf(Promise);
    });
    it('should resolve the returned promise with an object containing an array of the dependencies of given depRef item and all the resources for the given depRefItem', function () {
      return depMgr
        ._resolveDepReferenceDependencies(depRefItem, baseUrl)
        .then(function (result) {
          ...
          expect(result.dependencies[0].getId()).to.equal(
            'package3@1.0.0/util3'
          );
          expect(result.dependencies[1].getId()).to.equal(
            'package4@1.0.0/util4'
          );
        });
    });
    it('should use inline manifest from given dependency if there is any', function () {
      depRefItem.manifest = pkg1;
      return depMgr
        ._resolveDepReferenceDependencies(depRefItem, baseUrl)
        .then(function (result) {
    ...
    it('should use manifest from responseCache if there is already one for given webpackageId', function () {
      depMgr._responseCache.addItem(depRefItem.webpackageId, pkg1);
      return depMgr
        ._resolveDepReferenceDependencies(depRefItem, baseUrl)
        .then(function (result) {
          expect(_fetchManifestStub.callCount).to.equal(0);
    ...
    it('should add inline or requested manifest to response cache if there is no entry for corresponding webpackageId', function () {
      return depMgr
        ._resolveDepReferenceDependencies(depRefItem, baseUrl)
        .then(function (result) {
          expect(depMgr._responseCache.get(depRefItem.webpackageId)).to.eql(
            pkg1
          );
        });
    });
    it('should request manifest files from given baseUrl', function () {
      ...
      return depMgr
        ...
        .then(function (result) {
          expect(
            _fetchManifestStub.calledWith(
              baseUrl + depRefItem.webpackageId + '/manifest.webpackage'
            )
          );
        });
    });
  ...
  describe('#_logDependencyConflicts()', function () {
    ...
    beforeEach(function () {
      ...
      rootA.data = new DepMgr.DepReference({
        webpackageId: 'package1@1.0.0',
        artifactId: 'util1',
        referrer: null
      });

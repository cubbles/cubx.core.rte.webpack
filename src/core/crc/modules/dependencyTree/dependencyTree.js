...
DependencyTree.prototype._markExcludedNodesInSubtree = function (node) {
  ...
  this.traverseSubtreeBF(
    node,
    function (currentNode) {
      excludes.forEach(
        function (exclude) {
          if (
            exclude.webpackageId === currentNode.data.webpackageId &&
...
DependencyTree.prototype._removeDuplicate = function (duplicated, duplicate) {
  ...
  duplicated.data.referrer = duplicated.data.referrer.concat(
    duplicate.data.referrer
  );
 ...
 */
DependencyTree.prototype._removeConflictedNode = function (
  node,
  conflictedNode
) {
 ...
 * @private
 */
DependencyTree.prototype._getRelatedNodes = function (
  node,
  nodes,
  relationship
) {
  ...
  relatedNodes = nodes.filter(
    function (currentNode) {
      return (
        this._determineNodeRelationship(currentNode, node) === relationship
      );
    }.bind(this)
  );
 ...
 * @returns {object}
 */
DependencyTree.prototype._createConflictItem = function (
  node,
  conflictedNode,
  type,
  resolved
  ...
  this.traverseBF(
    function (node) {
      if (node.data.dependencyExcludes.length > 0) {
        this._markExcludedNodesInSubtree(node);
      }
    }.bind(this)
  );
 ...
 * @returns {object} DependencyTree itself
 */
DependencyTree.prototype.applyGlobalExclude = function (
  webpackageId,
  artifactId
) {
  this.traverseBF(
    function (node) {
      if (
        ...
        node.data.webpackageId === webpackageId
      ) {
        node.excluded = true;
...
DependencyTree.prototype.insertNode = function (node, parent, before) {
  ...
  if (parent && !(parent instanceof DependencyTree.Node)) {
    console.error(
      "parameter 'parent' needs to be an instance of DependencyTree.Node"
    );
    return;
  }
  if (before && !(before instanceof DependencyTree.Node)) {
    console.error(
      "parameter 'before' needs to be an instance of DependencyTree.Node"
    );
    return;
  }
  ...
  if (parent && before) {
    ...
    if (beforeIndex < 0) {
      console.warn(
        'Could not find given before node in parents child nodes array. Append given node to end of child nodes array.'
      );
      parent.children.push(node);
    } else {
      parent.children.splice(beforeIndex, 0, node);
    }
  } else if (parent && !before) {
    // if only parent is given, append given node to the end of parents children array
    node.parent = parent;
    parent.children.push(node);
  } else if (!parent && before) {
    // if only before is given insert node in rootNodes array as left neighbour of before node
    beforeIndex = -1;
    ...
    if (beforeIndex < 0) {
      console.warn(
        'Could not find given before node in rootNodes array. Append given node to end of rootNodes array.'
      );
    ...
    // neither parent nor before are given. So just append node to the end of rootNodes array
    node.parent = null;
    this._rootNodes.push(node);
  }
  ...
  this.traverseBF(
    function (node) {
      ...
      if (this.contains(node)) {
        var relatedNodes = {
          duplicates: this._getRelatedNodes(
            node,
            nodesBF,
            DependencyTree.NodeRelationship.ARTIFACT_DUPLICATE
          ),
          versionConflicts: this._getRelatedNodes(
            node,
            nodesBF,
            DependencyTree.NodeRelationship.ARTIFACT_VERSION_CONFLICT
          ),
          nameConflicts: this._getRelatedNodes(
            node,
            nodesBF,
            DependencyTree.NodeRelationship.ARTIFACT_NAME_CONFLICT
          ),
          distinct: this._getRelatedNodes(
            node,
            nodesBF,
            DependencyTree.NodeRelationship.DISTINCT_ARTIFACT
          )
        };
        ...
        if (relatedNodes.duplicates.length === 1) {
          ...
          var conflict = this._createConflictItem(
            relatedNodes.versionConflicts[0],
            node,
            DependencyTree.conflictTypes.VERSION,
            false
          );
          if (this._enableACR) {
          ...
          relatedNodes.distinct.length + relatedNodes.nameConflicts.length ===
          nodesBF.length
        ) {
          nodesBF.push(node);
        }
        ...
        if (relatedNodes.nameConflicts.length > 0) {
          relatedNodes.nameConflicts.some(
            function (conflictNode) {
              this._conflicts.push(
                this._createConflictItem(
                  conflictNode,
                  node,
                  DependencyTree.conflictTypes.NAME,
                  false
                )
              );
            }.bind(this)
...
DependencyTree.prototype.traverseSubtreeBF = function (rootNode, callback) {
  if (!(rootNode instanceof DependencyTree.Node)) {
    console.error(
      "Parameter 'rootNode' needs to be an instance of DependencyTree.Node"
    );
    return;
  }
  if (!callback || typeof callback !== 'function') {
...
DependencyTree.prototype.clone = function () {
  ...
  function clone (obj) {
    ...
    try {
      ...
      // The constructor failed, create without running it
...
DependencyTree.Node.prototype.equalsArtifactIgnoreVersion = function (node) {
  return (
    node.data.webpackageId.split('@')[0] + '/' + node.data.artifactId ===
    this.data.webpackageId.split('@')[0] + '/' + this.data.artifactId
  );
};
...
DependencyTree.Node.prototype.toJSON = function (includeResources) {
  ...
  if (this.usedBy.length > 0) {
    this.usedBy.forEach(function (child) {
      usedBy.push({
        webpackageId: child.data.webpackageId,
        artifactId: child.data.artifactId
      });
    });
  }

...
DependencyMgr.prototype.init = function () {
  ...
  this._depList = this._createDepReferenceListFromArtifactDependencies(
    rootDependencies,
    null
  );
...
DependencyMgr.prototype.run = function () {
  ...
  var rootDependencyExcludes =
    get(window, 'cubx.CRCInit.rootDependencyExcludes') || [];
  this._buildRawDependencyTree(
    this._createDepReferenceListFromArtifactDependencies(
      rootDependencies,
      null
    ),
    this._baseUrl
    ...
    .then(
      ...
      function (error) {
        console.error(
          'Error while building and processing DependencyTree: ',
          error
        );
      }
    );
...
DependencyMgr.prototype._calculateResourceList = function (depList) {
  ...
  for (var i = 0; i < depList.length; i++) {
    ...
    for (var j = 0; j < currentDepRef.resources.length; j++) {
      var qualifiedArtifactId =
        currentDepRef.webpackageId + '/' + currentDepRef.artifactId;
      var resource = this._createResourceFromItem(
        qualifiedArtifactId,
        currentDepRef.resources[j],
        this._runtimeMode,
        currentDepRef.referrer
      );
      if (resource) {
...
DependencyMgr.prototype._injectDependenciesToDom = function (resourceList) {
  ...
  if (
    typeof disableResourceInjection === 'undefined' ||
 ...
 */
DependencyMgr.prototype._buildRawDependencyTree = function (
  rootDependencies,
  baseUrl
) {
  ...
  return new Promise(
    function (resolve, reject) {
      ...
      (function resolveDependencies (dependencies, parentNodes) {
          ...
          function (dep) {
            try {
              resolutionsQueue.push(
                this._resolveDepReferenceDependencies(dep, baseUrl)
              );
            } catch (error) {
        ...
        Promise.all(resolutionsQueue).then(
          function (results) {
...
DependencyMgr.prototype._checkDepTreeForExcludes = function (depTree, baseUrl) {
  ...
  if (!(depTree instanceof DependencyTree)) {
    throw new TypeError(
      "parameter 'depTree' needs to be an instance of DependencyMgr.DependencyTree"
    );
  }
          ...
          function (results) {
            results.forEach(
              function (manifest, index) {
                try {
                  this._checkAndAddExcludesToDepReference(
                    nodes[index].data,
                    manifest
                  );
                } catch (e) {
 ...
 * @private
 */
DependencyMgr.prototype._checkAndAddExcludesForRootDependencies = function (
  node
) {
  // make some type checking of param node
  if (!(node instanceof DependencyTree.Node)) {
    throw new TypeError(
      "parameter 'node' needs to be an instance of DependencyTree.Node"
    );
  }
  ...
  var rootDep = rootDependencies.find(function (dep) {
    return (
      node.data.artifactId === dep.artifactId &&
      node.data.webpackageId === dep.webpackageId
    );
  });
 ...
 * @private
 */
DependencyMgr.prototype._checkAndAddExcludesToDepReference = function (
  depReference,
  manifest
) {
  ...
  if (!(depReference instanceof DependencyMgr.DepReference)) {
    throw new TypeError(
      "parameter 'depReference' needs to be an instance of DependencyMgr.DepReference"
    );
  }
  if (typeof manifest !== 'object') {
    throw new TypeError(
      "parameter 'manifest' needs to be a an object representing a webpackage.manifest"
    );
  }
  ...
  if (
    artifact.hasOwnProperty('dependencyExcludes') &&
    artifact.dependencyExcludes.length > 0
  ) {
    depReference.dependencyExcludes = depReference.dependencyExcludes.concat(
      JSON.parse(JSON.stringify(artifact.dependencyExcludes))
    );
  }
 ...
 * @private
 */
DependencyMgr.prototype._resolveDepReferenceDependencies = function (
  depReference,
  baseUrl
) {
  // check depReference
  if (!(depReference instanceof DependencyMgr.DepReference)) {
    throw new TypeError(
      "parameter 'depReference' need to be an instance of DependencyMgr.DepReference"
    );
  }
  // check baseUrl
  if (typeof baseUrl !== 'string') {
  ...
  return new Promise(
    function (resolve, reject) {
      ...
      var processManifest = function (manifest, cache) {
        ...
        try {
          if (
            artifact.hasOwnProperty('dependencies') &&
            artifact.dependencies.length > 0
          ) {
            dependencies = this._createDepReferenceListFromArtifactDependencies(
          ...
          console.error(
            "The artifact '" +
              depReference.artifactId +
              "' is not defined in manifest:",
      ...
      if (
        depReference.webpackageId &&
        this._responseCache.get(depReference.webpackageId) != null
      ) {
        // use manifest from responseCache if available
        processManifest(
          this._responseCache.get(depReference.webpackageId),
          false
        );
      } else if (typeof depReference.manifest === 'object') {
        // use inline manifest from depReference if set
        processManifest(depReference.manifest, true);
      } else {
        // default case: request manifest using ajax
 ...
 * @memberOf DependencyMgr
 */
DependencyMgr.prototype._createDepReferenceListFromArtifactDependencies = function (
  dependencies,
  referrer
) {
  var self = this;
  ...
  if (referrer && typeof referrer !== 'object') {
    console.warn(
      'Expect parameter "referrer" to be null or of type object: ',
      referrer,
      '. Will use "root" as fallback referrer'
    );
  ...
  dependencies.forEach(function (dependency) {
    ...
    if (
      !(
        ...
        dependency.hasOwnProperty('artifactId') &&
        typeof dependency.artifactId === 'string'
      )
    ) {
      console.error(
        'Expected parameter to be an object containing at least string property "artifactId": ',
        dependency
      );
      valid = false;
    }
    ...
    if (
      dependency.hasOwnProperty('manifest') &&
      typeof dependency.manifest !== 'object'
    ) {
      console.error(
        'Expected parameter to be an object containing at least string property "artifactId": ',
        dependency
 ...
 */
DependencyMgr.prototype._createResourceFromItem = function (
  qualifiedArtifactId,
  item,
  runtimeMode,
  referrer
  ...
  if (
    !item.hasOwnProperty('prod') ||
    !item.hasOwnProperty('dev') ||
    typeof item.prod !== 'string' ||
    typeof item.dev !== 'string'
  ) {
    throw new TypeError(
      'parameter "item" needs to have string properties "prod" and "dev"'
    );
  }

  if (qualifiedArtifactId === null || typeof qualifiedArtifactId !== 'string') {
    throw new TypeError(
      'parameter "qualifiedArtifactId" needs to be of type string'
    );
  }
  ...
  if (
    item[runtimeMode].indexOf('http') === 0 ||
    item[runtimeMode].indexOf('blob') === 0
  ) {
    if (allowAbsoluteResourceUrls) {
      file = item[runtimeMode];
    } else {
      console.warn(
        'The following resource can not be loaded since the use of absolute urls is not allowed by default: ' +
          item[runtimeMode]
      );
      return;
    }
  ...
  if (!resMetaObj.fileType) {
    console.warn(
      'The following resource will be ignored, because the type of the resource is unkown. It should be "js", "html" or "css". (' +
        item[runtimeMode] +
        ')'
    );
    return;
  }
  return new DependencyMgr.Resource(
    resMetaObj.fileName,
    resMetaObj.fileType.name,
    referrer
  );
};
...
DependencyMgr.prototype._determineResourceType = function (fileName) {
  ...
  var fileEnding =
    paramType || fileName.split('.')[fileName.split('.').length - 1];
 ...
 * @private
 */
DependencyMgr.prototype._determineWebpackageId = function (
  dependency,
  referrer
) {
  if (
    dependency.hasOwnProperty('webpackageId') &&
    typeof dependency.webpackageId === 'string'
  ) {
    return dependency.webpackageId;
  } else if (
    referrer &&
    typeof referrer === 'object' &&
    referrer.hasOwnProperty('webpackageId') &&
    referrer.hasOwnProperty('artifactId')
  ) {
    ...
    console.error(
      'Could not determine webpackageId for dependency: ',
      dependency,
      ' and referrer: ',
      referrer
    );
 ...
 * @private
 */
DependencyMgr.prototype._determineWebpackageIdsForRootRependencies = function (
  rootDependencies
) {
  ...
  } else if (pathname.split('/')[1] === 'localstore') {
    // if in locale request a the virtual store exists
    webpackageId = pathname.split('/')[2];
  } else {
    // if in locale request without store
 ...
 * @private
 */
DependencyMgr.prototype._getManifestForDepReference = function (
  depReference,
  baseUrl
) {
  // make some type checking
      ...
      if (this._responseCache.get(webpackageId) != null) {
        ...
        var url =
          baseUrl.lastIndexOf('/') !== baseUrl.length - 1
            ? baseUrl + '/'
            : baseUrl;
        this._fetchManifest(url + webpackageId + '/manifest.webpackage').then(
          function (result) {
            resolve(result.data);
          },
          function (error) {
...
DependencyMgr.DepReference = function (initObject) {
  ...
  this.equals = function (item) {
    return (
      item.webpackageId + item.artifactId ===
      this.webpackageId + this.artifactId
    );
  };
  ...
  (function (referrer) {
    if (
      ...
      console.warn(
        "DepManager received referrer of unexpected type '" +
          typeof referrer +
          "'"
      );
    }

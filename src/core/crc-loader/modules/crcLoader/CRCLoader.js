...
CRCLoader.prototype.run = function () {
  document.addEventListener(
    'crcDepMgrReady',
    this._includeMainScript.bind(this)
  );
  var action = function () {
    ...
    this._cubxCRCInitRootDependenciesOriginLength = cubx.CRCInit
      .rootDependencies
      ? cubx.CRCInit.rootDependencies.length
      : 0;
    // keep origin length of cubx.CRCInit.rootDependencyExcludes
    this._cubxCRCInitRootDependencyExludesOriginLength = cubx.CRCInit
      .rootDependencyExcludes
      ? cubx.CRCInit.rootDependencyExcludes.length
      : 0;
...
CRCLoader.prototype._checkRootDependencies = function () {
  if (
    !cubx.CRCInit.hasOwnProperty('rootDependencies') ||
    typeof cubx.CRCInit.rootDependencies === 'undefined'
  ) {
    ...
    if (typeof dep === 'object' && dep.artifactId) {
      ...
      console.warn(
        'The dependency definition "' +
          dep +
          '" will be removed, because it is not valid for this rte version. A valid dependency definition should be an object, and it should to have at least the attribute artifactId'
      );
    }
  });
...
CRCLoader.prototype._addComponentDependenciesToRootDependencies = function () {
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    if (!this._isDependencyInRootDependencies(element)) {
...
CRCLoader.prototype._isDependencyInRootDependencies = function (element) {
  ...
  if (foundling && foundling.webpackageId !== webpackageId) {
    ...
    console.warn(
      'Ambiguous webpackageId definition for artifact "' +
        element.tagName.toLowerCase() +
        '" The defined webpackageId (' +
        webpackageId +
        ') is inconsistent with an earlier definition of webpackageId (' +
        foundling.webpackagageId +
        '). Use the dependency with webpackageId "' +
        foundling.webpackageId +
        '" ignore webpackageId "' +
        webpackageId +
        '".'
    );

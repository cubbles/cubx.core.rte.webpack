 ...
 * @public
 */
DependencyTagTransformer.prototype.addDependenciesAndExcludesToRootDependencies = function (
  crcLoader
) {
  if (
    !cubx ||
    ...
    cubx.CRCInit.rootDependencies.length === 0
  ) {
    if (window.cubx.CRCInit.runtimeMode === 'dev') {
      console.warn(
        'No dependencies for any components defined. It won\'t be searched for "cubx-dependencies" or "cubx-depenedency-excludes".'
      );
    }
    return;
  }
  ...
  var artifactElements = crcLoader._crcRoot.querySelectorAll(
    artifactList.join(',')
  );
  ...
  for (i = 0; i < len; i++) {
    ...
    this._addToCubxCRCInit(
      foundling,
      element,
      crcLoader._cubxCRCInitRootDependenciesOriginLength
    );
  }
};
// -----------------------------------------------------------------------------------
 ...
 * @private
 */
DependencyTagTransformer.prototype._findDependenciesAndExcludesInElement = function (
  element
) {
  var i;
  ...
  for (i = 0; i < len; i++) {
    if (childElements[i].tagName === 'CUBX-DEPENDENCIES') {
      dependencies = this._filterChildElements(
        childElements[i],
        'cubx-dependency'
      );
    }
    if (childElements[i].tagName === 'CUBX-DEPENDENCY-EXCLUDES') {
      excludes = this._filterChildElements(
        childElements[i],
 ...
 * @private
 */
DependencyTagTransformer.prototype._filterChildElements = function (
  element,
  childElementName
) {
  var list = [];
 ...
 * @private
 */
DependencyTagTransformer.prototype._addToCubxCRCInit = function (
  deps,
  artifactElement,
  originLength
) {
  ...
  deps.dependencies.forEach(
    function (elem, index) {
      var artifactId = elem.getAttribute('artifact-id');
      if (!artifactId) {
        console.error(
          'Missing artifactId: The artifact-id attribute is mandatory for each "' +
        ...
        cubx.CRCInit.rootDependencies.splice(
          cubx.CRCInit.rootDependencies.length - originLength,
          0,
          dep
        );
      }
 ...
 * @private
 */
DependencyTagTransformer.prototype._createDependency = function (
  element,
  artifactElement
) {
...
export default DependencyTagTransformer;

...
CIF.prototype._initForCRCRoot = function (node) {
  ...
  node.addEventListener(
    EventFactory.types.CIF_ALL_COMPONENTS_READY,
    function () {
...
CIF.prototype._createObserverObject = function () {
  ...
  if (Object.keys(this.CRC.getCache().getAllComponents()).length === 0) {
    console.warn(
      'Can not initialise Mutation Summary (observer), because the component cache is empty.'
    );
    return;
  }
  ...
  if (this.CRC.getRuntimeMode() === 'dev') {
...
CIF.prototype._detectMutation = function (summaries) {
  ...
  componentChangeSummary.removed.forEach(function (removedEl) {
    cif._handleRemovedCubble(
      removedEl,
      componentChangeSummary.getOldParentNode(removedEl)
    );
  });
  ...
  componentChangeSummary.removed.forEach(function (removedEl) {
    if (removedEl.tagName === 'CUBX-CORE-CONNECTIONS') {
      cif._handleRemovedConnections(
        removedEl,
        componentChangeSummary.getOldParentNode(removedEl)
      );
    }
    if (removedEl.tagName === 'CUBX-CORE-CONNECTION') {
      var parent = componentChangeSummary.getOldParentNode(removedEl)
        .parentNode;
      if (parent) {
        // handle removed connection just, if the tag is removed from DOM directly. (The oldParentNode is in DOM)
...
CIF.prototype._determineMemberIdAndRuntimeId = function (element) {
  ...
  var resolvedComponentManifest = this.CRC.getResolvedComponent(
    componentName
  );
  ...
  var componentId =
    resolvedComponentManifest.webpackageId +
    '/' +
    resolvedComponentManifest.artifactId;
  var runtimeId = componentId + '.' + memberId;

  if (originRuntimeId && runtimeId !== originRuntimeId) {
    console.warn(
      'The "runtime-id" attribute is invalid: (' +
        originRuntimeId +
        '). It will be set to:' +
        runtimeId
    );
...
CIF.prototype._handleRemovedCubble = function (element, oldParentNode) {
  if (!element || element.generatedByCif) {
    // no remove handling for elements generated by cif
    return;
  }
  var context;
  if (oldParentNode && oldParentNode.Context) {
    ...
    if (
      !context &&
      this.getCRCRootNode() &&
      this.getCRCRootNode().Context &&
      this.getCRCRootNode().Context._components.find(function (comp) {
        return comp === element;
      })
    ) {
  ...
  if (context) {
    ...
    console.warn(
      'No context found for removed cubble "' +
...
CIF.prototype._processElementFromQueue = function () {
  ...
  if (!elements) {
    // end the process, if no element in the queue
    return;
  }
...
CIF.prototype._handleAddedConnection = function (connectionElement) {
  ...
  if (connectionElement.getType() === 'internal') {
    console.warn(
      "Can't handle added element, because it is an internal connection. Added connection:",
      connectionElement
    );
    return;
  }
  ...
  if (!connections || connections.tagName !== 'CUBX-CORE-CONNECTIONS') {
    console.warn(
      'Can\'t handle the added element. A "cubx-core-connection" element must be a child of a "cubx-core-connections" element. Added element:',
      connectionElement
    );
    return;
  }
  ...
  if (
    !this.CRC.getCache().getComponentCacheEntry(
      component.tagName.toLowerCase()
    )
  ) {
    console.warn(
      'Can\'t handle the added element. A "cubx-core-connections" element must be a child an cubble. The current parent element:',
      component
    );
    return;
  }
  ...
  if (parent !== this.getCRCRootNode()) {
    console.warn(
      "Can't handle added element. The connection in not in scope of root context. It is only allowed add connection element dynamically for the root context. Added connection:",
      connectionElement
    );
    return;
  }
  if (parent) {
    ...
    parent.Context.getConnectionMgr().createConnectionFromComponent(
      component,
      connectionElement
    );
  }
 ...
 * @private
 */
CIF.prototype._handleRemovedConnections = function (
  connectionsElement,
  oldParentNode
) {
 ...
 * @private
 */
CIF.prototype._handleRemovedConnection = function (
  connectionElement,
  parentCubble
) {
  if (connectionElement && connectionElement.generatedByCif) {
  ...
  if (connectionElement.getType() === 'internal') {
    console.warn(
      "Can't handle removed element, because it is an internal connection. Removed connection:",
      connectionElement
    );
    return;
  }
  ...
  if (
    ...
    console.warn(
      'Can\'t handle the removed connection element. A "cubx-core-connections" element must be a child an cubble. The current parent element:',
      component
    );
    return;
  }
  ...
  if (parent !== this.getCRCRootNode()) {
    console.warn(
      "Can't handle removed element. The connection in not in scope of root context. It will only processed a removed connection element in the scope of root context. Removed connection:",
      connectionElement
    );
    return;
  }
...
CIF.prototype._fireAllComponentsReady = function (node) {
  ...
  var evt = this.getEventFactory().createEvent(
    EventFactory.types.CIF_ALL_COMPONENTS_READY
  );
...
CIF.prototype._isNodeInCache = function (node) {
  ...
  return this.CRC.getCache().getWebpackageDocument(tagname);
};
...
CIF.prototype._initCubxElements = function (rootNode) {
  ...
  var filter = {
    acceptNode: function (currNode) {
      ...
      if (
        currNode.nodeType === currNode.ELEMENT_NODE &&
        currNode.tagName.indexOf('-') > -1 &&
        except.indexOf(currNode.tagName) === -1
      ) {
  ...
  while (walker.nextNode()) {
    ...
    var manifest = this.CRC.getCache().getComponentCacheEntry(
      this._getTagname(curNode)
    );
...
CIF.prototype._initCubxElementsInRoot = function (element, memberIds) {
  ...
  if (_.includes(memberIds, memberId)) {
    throw new Error(
      'The member-id "' +
        memberId +
        '" is not uniq. The same memberId used before.'
    );
  }
  ...
  if (this._isElementaryComponent(element)) {
    ...
    element = this._createDOMTreeFromManifest(
...
CIF.prototype._updateCubxCoreInit = function (element, initOrder) {
  ...
  if (cubxCoreInit) {
    var cubxCoreSlotinits = cubxCoreInit.querySelectorAll(
      'cubx-core-slot-init'
    );
    var removeList = [];
    var i;
    for (i = 0; i < cubxCoreSlotinits.length; i++) {
      if (
        cubxCoreSlotinits[i].getType() &&
        cubxCoreSlotinits[i].getType() === 'internal'
      ) {
    ...
    for (i = removeList.length - 1; i >= 0; i--) {
      console.error(
        'Internal initialisation in root level per tags not allowed. The following init tag will be deleted: ',
        removeList[i]
      );
...
CIF.prototype._updateCubxCoreConnections = function (element) {
  ...
  for (i = cubxCoreConnectionList.length - 1; i >= 0; i--) {
    ...
    if (con.getType() && con.getType() === 'internal') {
      console.error(
        'Internal initialisation in root level per tags not allowed. The following connection tag will be deleted: ',
        con
      );
...
CIF.prototype._initSlots = function (node) {
  ...
  var cifInitStartEvent = this.getEventFactory().createEvent(
    EventFactory.types.CIF_INIT_START
  );
  ...
  if (this.CRC.getRuntimeMode() === 'dev') {
    console.log(
      ...
      this._initializer._initList.length,
      ')',
      this._initializer._initList
    );
  }
  this._initializer.initSlots();
  var cifInitReadyEvent = this.getEventFactory().createEvent(
    EventFactory.types.CIF_INIT_READY
  );
...
CIF.prototype._registerCompoundComponentElement = function (name) {
  if (
    typeof name !== 'string' ||
    (typeof name === 'string' && name.indexOf('-') < 1)
  ) {
    throw new TypeError(
      'parameter name needs to be of type "string" and needs to contain a "-"'
    );
  }
  ...
  function getConstructor () {
    ...
    Object.setPrototypeOf(
      CompoundComponentClass.prototype,
...
CIF.prototype._registerConnectionElements = function () {
  function getConnectionConstructor () {
    ...
    ConnectionClass.prototype.setDestination = function (slot, memberIndex) {
      var destination =
        memberIndex != null ? memberIndex + ':' + slot : 'parent:' + slot;
...
CIF.prototype._createDOMTreeFromManifest = function (manifest, root) {
  if (manifest.artifactId !== root.tagName.toLowerCase()) {
    throw new Error(
      'The manifest has referenced a different tag (' +
        manifest.artifactId +
        '), than the tag in the CRC container (' +
 ...
 * @private
 */
CIF.prototype._attachMembersFromTemplate = function (
  root,
  rootManifest,
  template,
  deeplevel
  ...
  for (var i = 0; i < elementList.length; i++) {
    ...
    var cache = this.CRC.getCache();
    var manifest = cache.getComponentCacheEntry(
      component.tagName.toLowerCase()
    );
    ...
    if (!manifest) {
      if (this.CRC.getRuntimeMode() === 'dev') {
        console.warn(
          'The component "' +
            component.tagName.toLowerCase() +
            '" not referenced yet. Please include the dependency definition to the component "' +
            root.tagName.toLowerCase() +
            '".'
        );
      }
    ...
    if (
      manifest &&
      !_.find(rootManifest.members, {
        memberId: component.getAttribute('member-id-ref')
      })
    ) {
      if (this.CRC.getRuntimeMode() === 'dev') {
        console.warn(
          'The cubbles "' +
            manifest.artifactId +
            '" is not a member component of "' +
            rootManifest.artifactId +
            '".'
        );
      }
      continue;
 ...
 * @private
 */
CIF.prototype._attachMembersFromManifest = function (
  root,
  rootManifest,
  deeplevel
) {
  ...
  for (var i = 0; i < members.length; i++) {
    ...
    if (!this._isElementaryComponentInManifest(currentMember)) {
      // currentMember is a compound component
      var constructor = this.getCompoundComponentElementConstructor(
    ...
    var componentId =
      currentMember.webpackageId + '/' + currentMember.artifactId;
    // create runtimeId
    runtimeId =
      rootRuntimeId + ':' + componentId + '.' + currentMember.memberId;
    component.setAttribute('runtime-id', runtimeId);
    //  add  Element to list of waiting for Ready Elements --> Ready-Event necessary
    this._componentReady[runtimeId] = {
      ready: false
    ...
    while (node) {
      ...
      if (
        ...
        if (manifest && !node.getAttribute('member-id-ref')) {
          console.warn(
            'The attribute "member-id-ref" is missing in template of the component "' +
              rootManifest.artifactId +
              '" on tag "' +
              manifest.artifactId +
              '".'
...
CIF.prototype._createConnectionElement = function (root, connection) {
  ...
  if (connection.source.hasOwnProperty('memberIdRef')) {
    ...
    while (node) {
      ...
      if (
        _.indexOf(root.Context._components, node) > -1 &&
        node.getAttribute('member-id') === connection.source.memberIdRef
      ) {
        sourceEl = node;
        node = null;
      } else {
  ...
  cubxConnectionEl.setDestination(
  ...
  if (
    typeof connection.copyValue !== 'undefined' &&
    typeof connection.copyValue === 'boolean'
  ) {
    cubxConnectionEl.setCopyValue(connection.copyValue);
  }

  if (
    typeof connection.repeatedValues !== 'undefined' &&
    typeof connection.repeatedValues === 'boolean'
  ) {
    cubxConnectionEl.setRepeatedValues(connection.repeatedValues);
  }
  if (
    typeof connection.hookFunction !== 'undefined' &&
    typeof connection.hookFunction === 'string'
  ) {
...
CIF.prototype._createSlotInitElement = function (root, slotInit, order) {
  ...
  if (slotInit.hasOwnProperty('memberIdRef')) {
    ...
    while (node) {
      if (
        _.indexOf(root.Context._components, node) > -1 &&
        node.getAttribute('member-id') === slotInit.memberIdRef
      ) {
        el = node;
        node = null;
      } else {
  ...
  if (!el) {
    console.error(
      'the "_createSlotInitElement" method not found the member element: ',
      root,
      'slotInit',
      slotInit
    );
...
CIF.prototype._isComponentFromType = function (manifest, type) {
  if (
    manifest &&
    manifest.hasOwnProperty('artifactType') &&
...
CIF.prototype._isCompoundComponent = function (component) {
  var manifest = this.CRC.getResolvedComponent(
    this._getTagname(component)
  );
...
CIF.prototype._ready = function (node) {
  ...
  if (node.Context) {
    ...
    if (this._isInitialProcessing()) {
      cifReadyEvent = this.getEventFactory().createEvent(
        EventFactory.types.CIF_READY
      );
    }
    if (this._isObserverTriggeredProcessing()) {
      cifReadyEvent = this.getEventFactory().createEvent(
        EventFactory.types.CIF_DOM_UPDATE_READY
      );
    }
    if (cifReadyEvent) {
...
CIF.prototype._isModelVersionSupported = function (modelVersion) {
  var correctedModelVersion = this.CRC.getNormedModelVersion(
    modelVersion
  );
...
CIF.prototype._checkModelVersionForAllComponents = function () {
  ...
  for (var comp in components) {
    if (components.hasOwnProperty(comp)) {
      ...
      if (
        warnings.indexOf(id) === -1 &&
        !this._isModelVersionSupported(component.modelVersion)
      ) {
        warnings.push(id);
        allOk = false;
        console.warn(
          'Webpackage with id ' +
            ...
            '. This CIF supports the following modelVersion(s): ' +
            this._supportedModelVersionList
        );
      }
    }
  }

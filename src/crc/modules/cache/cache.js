...
Cache.prototype.addComponentCacheEntry = function (document, artifactId) {
  ...
  if (!this._crc._isModelVersionSupported(document.modelVersion)) {
    console.warn(
      'Webpackage with id ' +
        ...
        '. This CRC (version=' +
        this._crc._version +
        ') supports the following modelVersion(s): ' +
  ...
  if (artifactId) {
    ...
    throw new TypeError(
      'Undefined artifactId: The parameter artifactId is obligatory.'
    );
  }
};
...
Cache.prototype._addComponentCacheEntry = function (artifactEntry) {
  var artifactId = artifactEntry.artifactId;
  var id = artifactEntry.webpackageId + '/' + artifactId;
  var otherArtifact = this.getComponentCacheEntry(artifactId);
  if (
    otherArtifact &&
    otherArtifact.webpackageId !== artifactEntry.webpackageId
  ) {
    console.warn(
      'Trying to store an ambiguous component "' +
        id +
        '" in the componentCache. ' +
        'In the componentCache exists already another artifact with the same artifactId, but a ' +
        'different webpackageId: "' +
        otherArtifact.webpackageId +
        '/' +
...
Cache.prototype._findComponentInDocument = function (document, artifactId) {
  ...
  if (document.artifacts && document.artifacts.compoundComponents) {
    for (i = 0; i < document.artifacts.compoundComponents.length; i++) {
      if (document.artifacts.compoundComponents[i].artifactId === artifactId) {
        document.artifacts.compoundComponents[i].artifactType =
          ArtifactTypes.COMPOUND_COMPONENT;
        document.artifacts.compoundComponents[i].modelVersion =
          document.modelVersion;
  ...
  if (document.artifacts && document.artifacts.elementaryComponents) {
    for (i = 0; i < document.artifacts.elementaryComponents.length; i++) {
      if (
        document.artifacts.elementaryComponents[i].artifactId === artifactId
      ) {
        document.artifacts.elementaryComponents[i].artifactType =
          ArtifactTypes.ELEMENTARY_COMPONENT;
        document.artifacts.elementaryComponents[i].modelVersion =
          document.modelVersion;
...
var ArtifactTypes = {
  ...
  isComponent: function (type) {
    if (
      (type && type === this.COMPOUND_COMPONENT) ||
      type === this.ELEMENTARY_COMPONENT
    ) {
      return true;
    }

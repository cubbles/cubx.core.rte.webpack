...
import DepMgr from '../dependencyManager/depMgr';
...
var CRC = function () {
   ...
   * @private
   */
  this._componentResolver = new ComponentResolver(
    this._cache,
...
CRC.prototype.getResolvedComponent = function (artifactId) {
  ...
  if (typeof artifactId === 'string') {
    ...
    throw new TypeError(
      'Parameter artifactId "' + artifactId + '" not a string.'
    );
  }
...
CRC.prototype.fireBeforeResourceInjectionEvent = function () {
  var event = new CustomEvent(this._beforeResourceInjectionEventName, {
    bubbles: true
  });
  if (this.getCRCElement()) {
    this.getCRCElement().dispatchEvent(event);
...
CRC.prototype._resolveComponent = function (artifactId) {
  ...
  cache.setResolvedComponent(
    artifact.webpackageId + '/' + artifact.artifactId,
    resolvedComponent
  );

...
import depList from './dependencyManager/dependencies';
...
describe('DependencyManager', function () {
  ...
  describe('#_createDepReferenceListFromArtifactDependencies()', function () {
    ...
    it('should create list of DepReference items from given list of dependencies', function () {
      ...
      var depList = depMgr._createDepReferenceListFromArtifactDependencies(
        dependencies,
        referrer
      );
    ...
    it('should set referrer to "root" if param referrer is set to null', function () {
      var item = depMgr._createDepReferenceListFromArtifactDependencies(
        dependencies,
        null
      )[0];
  ...
  describe('#_injectDependenciesToDom()', function () {
    ...
    before(function () {
      depMgr = new DepMgr();
      injectedResources = [];
      appendStylesheetStub = sinon
        .stub(utils.DOM, 'appendStylesheetToHead')
        .callsFake(function (file) {
          injectedResources.push(file);
        });
      appendHtmlImportStub = sinon
        .stub(utils.DOM, 'appendHtmlImportToHead')
        .callsFake(function (file) {
          if (!file.startsWith('blob')) {
            injectedResources.push(file);
          }
        });
      appendScriptStub = sinon
        .stub(utils.DOM, 'appendScriptTagToHead')
        .callsFake(function (file) {
          injectedResources.push(file);
        });
      createScriptForFireEventSpy = sinon.spy(
        depMgr,
        '_createScriptForFireEvent'
      );
      resourceList = [
        new DepMgr.Resource('test.html', 'htmlImport', [
          { webpackageId: 'referrer1', artifactId: 'artifact1' },
          'referrer2/artifact2'
        ]),
        new DepMgr.Resource('test.css', 'stylesheet', [
          { webpackageId: 'referrer3', artifactId: 'artifact3' }
        ]),
        new DepMgr.Resource('test.js', 'javascript', [
          { webpackageId: 'referrer4', artifactId: 'artifact4' }
        ])
      ];
    });
    beforeEach(function () {
    ...
    it('should append all html import resources of given resource list into DOM using utils.DOM api including all referrers of each resource', function () {
      ...
      secondCall.args[1].should.be.eql([
        'referrer1/artifact1',
        'referrer2/artifact2'
      ]);
    ...
    it('should append all css resources of given resource list into DOM using utils.DOM api including all referrers of each resource', function () {
      ...
      expect(
        appendStylesheetStub.calledWith('test.css', ['referrer3/artifact3'])
      ).to.be.true;
    });
    it('should append all javascript resources of given resource list into DOM using utils.DOM api including all referrers of each resource', function () {
      ...
      expect(appendScriptStub.calledWith('test.js', ['referrer4/artifact4'])).to
        .be.true;
    });
